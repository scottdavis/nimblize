...
class AlterTable {
  ...
  public function foreign_key($column_name, $ref_table, $ref_column, $options = array('on_update' => 'CASCADE', 'on_delete' => 'CASCADE')) {
    ...
    $fk = 'CONSTRAINT ' . $name . ' FOREIGN KEY (' . Migration::quote_column_name($column_name) . ') REFERENCES ' . Migration::quote_table_name($ref_table) . ' (' . Migration::quote_column_name($ref_column) . ')' . ' ON UPDATE ' . $options['on_update'] . ' ON DELETE ' . $options['on_delete'];
